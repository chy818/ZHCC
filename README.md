...
玄语编译器自动管理闭包内存：
- 闭包创建时自动分配内存
- 函数返回前自动调用 `rt_closure_destroy()` 释放内存
- 无需手动管理闭包生命周期
...
```xy
函数 fibonacci(整数 n, 整数 a, 整数 b): 整数 {
    若 n == 0 则 {
        返回 a
    }
    若 n == 1 则 {
